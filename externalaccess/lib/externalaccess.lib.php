...
function print_ticketTable($socId = 1)
{
    global $langs,$db;
    ...
    $sql = 'SELECT rowid ';
    $sql.= ' FROM `'.MAIN_DB_PREFIX.'ticketsup` t';
    ...
    if(!empty($tableItems))
    {
        ...
        print ' <th class="text-center" >'.$langs->trans('fk_statut').'</th>';
		print ' <th class="text-center" >'.$langs->trans('category_code ').'</th>';
        print ' <th class="text-center" >'.$langs->trans('severity_code ').'</th>';
		print ' <th class="text-center" >'.$langs->trans('category_code ').'</th>'; 	
		print ' <th class="text-center" >'.$langs->trans('fk_project').'</th>';
       ...
       // print ' <th class="text-center" >'.$langs->trans('Amount_TTC').'</th>';
        ...
        load_last_main_doc($object);
            $dowloadUrl = $context->getRootUrl().'script/interface.php?action=downloadticket&id='.$object->id;
...
function print_fichinterTable($socId = 1)
{
    global $langs,$db;
    $context = Context::getInstance();
...
/*pm17*/
function print_contractsTable($socId = 1)
{
    global $langs,$db;
    $context = Context::getInstance();
    ...
    $sql = 'SELECT rowid ';
    $sql.= ' FROM `'.MAIN_DB_PREFIX.'contrat` c';
    $sql.= ' WHERE fk_soc = '. intval($socId);
    $sql.= ' AND fk_statut > 0';
    ...
    if(!empty($tableItems))
    {
        ...
        print '<table id="contrat-list" class="table table-striped" >';
        ...
        foreach ($tableItems as $item)
        {
            ...
            $object->fetch($item->rowid);
        load_last_main_doc($object);
            $dowloadUrl = $context->getRootUrl().'script/interface.php?action=downloadContract&id='.$object->id;
            //var_dump($object); exit;
            /*$totalpaye = $object->getSommePaiement();
            $totalcreditnotes = $object->getSumCreditNotesUsed();
            $totaldeposits = $object->getSumDepositsUsed();
...
function print_agendaTable($socId = 1)
{
    ...
    if(!empty($tableItems))
    {
        ...
        print '</table>';
        $jsonUrl = $context->getRootUrl().'script/interface.php?action=getagendaList';
    ?>
    <script type="text/javascript" >
     $(document).ready(function(){
         $("#invoice-list").DataTable({
